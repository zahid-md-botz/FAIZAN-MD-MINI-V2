...
async function saveSessionToMongoDB(number, credsText) {
    ...
    await col.updateOne(
        { _id: sanitize(number) },
        // A fresh save clears the rejected flag: re-pairing is what repairs a session.
 ...
 *  Sessions WhatsApp rejected are excluded: auto-starting them produced a
 *  connect -> 401 -> wipe loop that destroyed freshly paired logins. */
...
/** Flag a session as rejected instead of deleting it (the old behaviour lost logins). */
